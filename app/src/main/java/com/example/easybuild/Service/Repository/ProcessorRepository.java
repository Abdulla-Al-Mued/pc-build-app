...
public class ProcessorRepository {
    ...
    private static ProcessorRepository instance = null;
    private static Context ctx;
    private static MutableLiveData<Root> processors;

    public static ProcessorRepository getInstance(Context context){
    ...
    public MutableLiveData<Root> processors(){

        if (processors == null)
            processors = new MutableLiveData<>();


        ApiService apiService = RetroInstance.getRetroInstance().create(ApiService.class);
        Call<Root> call = apiService.allProcessor();
        call.enqueue(new Callback<Root>() {
            @Override
            public void onResponse(Call<Root> call, Response<Root> response) {
                if (response.body()!= null){
                    processors.postValue(response.body());
                }
                else
