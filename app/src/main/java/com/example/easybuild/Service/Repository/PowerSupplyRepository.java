...
public class PowerSupplyRepository {
    ...
    private static PowerSupplyRepository instance = null;
    private static Context ctx;
    private static MutableLiveData<Root> powerSupply;

    public static PowerSupplyRepository getInstance(Context context){

        if (instance == null){
            ctx = context;
            instance = new PowerSupplyRepository();
        }
    ...
    public MutableLiveData<Root> powerSupply(){
        ...
        if (powerSupply==null){
            powerSupply = new MutableLiveData<>();
        }

        ApiService apiService = RetroInstance.getRetroInstance().create(ApiService.class);
        Call<Root> call = apiService.allPowerSupply();
        call.enqueue(new Callback<Root>() {
            @Override
            public void onResponse(Call<Root> call, Response<Root> response) {
                if (response.body()!= null){
                    powerSupply.postValue(response.body());
                }
                else
